...
def ExtractData(filename):
    ...
    ## Metadata extraction
    file.readline()
    L_list =  list(map(int, file.readline().strip('][\n').split(', ')))
    ...
    X_list = [np.zeros((len(T_eval), len(L_list), N+2, len(init_conf_list), len(E_b_list), len(K_s_list), len(n_L_list), len(m_L_list), len(Lambda_list), len(X_flow_field_list))) for N in N_list]
    file.readline()
    file.readline()
                                ...
                                for Lambda_index in range(len(Lambda_list)):
                                    for X_flow_field_index in range(len(X_flow_field_list)):
                                        ...
                                        for t_index in range(len(T_eval)):
                                            X_list[N_index][t_index, L_index, N_index, init_conf_index, E_b_index, K_s_index, n_L_index, m_L_index, Lambda_index, X_flow_field_index] = list(map(float, file.readline().strip('][\n').lstrip().replace('  ', ' ').split(' ')))
                                        file.readline()
    
    parameters_list = [L_list, N_list, init_conf_list, E_b_list, K_s_list, n_L_list, m_L_list, Lambda_list, X_flow_field_list, mu, r, h, T_span, T_eval]
    return parameters_list, X_list


def ParamAndShape(param_indices, param_list, X_list):

    param = [param_list[k][param_indices[k]] for k in range(len(param_indices))] + param_list[-5:]
    # X_list[N_index][t_index, L_index, N_index, init_conf_index, E_b_index, K_s_index, n_L_index, m_L_index, Lambda_index, X_flow_field_index] = list(map(float, file.readline().strip('][\n').lstrip().replace('  ', ' ').split(' ')))
    X = X_list[param_indices[1]][:, param_indices[0], :, param_indices[2], param_indices[3], param_indices[4], param_indices[5], param_indices[6], param_indices[7], param_indices[8]]
    return param, X
