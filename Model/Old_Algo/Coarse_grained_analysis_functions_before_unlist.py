...
def ExtractData(filename):
    ...
    X_list = [np.zeros((len(T_eval), len(L_list), N+2, len(init_conf_list), len(E_b_list), len(K_s_list), len(n_L_list), len(m_L_list))) for N in N_list]
    file.readline()
    file.readline()
                        ...
                        for n_L_index in range(len(n_L_list)):
                            for m_L_index in range(len(m_L_list)):
                                    ...
                                    for t_index in range(len(T_eval)):
                                        X_list[N_index][t_index, L_index, :, init_conf_index, E_b_index, K_s_index, n_L_index, m_L_index] = list(map(float, file.readline().strip('][\n').lstrip().rstrip().replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').split(' ')))
                                    file.readline()
    
    parameters_list = [L_list, N_list, init_conf_list, E_b_list, K_s_list, n_L_list, m_L_list, mu, r, h, T_span, T_eval, Lambda_list, Zeta_list, Nus_list]
...
def ParamAndShape(param_indices, param_list, X_list):
    ...
    X = X_list[param_indices[1]][:, param_indices[0], :, param_indices[2], param_indices[3], param_indices[4], param_indices[5], param_indices[6]]
    return param, X
