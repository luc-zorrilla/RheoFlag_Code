...
#         return [{
#             "T_span": (1e1, 2e1),
#             "T_eval": np.linspace(1e1, 2e1, int(1e2)), # minimum two elements here.
#             "method": "BDF",
#             "T_sim_max": 300,
#         },
#         {
#             "T_span": (1e0, 2e0),
#             "T_eval": np.linspace(1e0, 2e0, int(1e2)), # minimum two elements here.
#             "method": "BDF",
#             "T_sim_max": 300,
#         },        
#         {
#             "T_span": (1e2, 2e2),
#             "T_eval": np.linspace(1e2, 2e2, int(1e2)), # minimum two elements here.
#             "method": "BDF",
#             "T_sim_max": 300,
#         },
#         ]
...
#         for ext_params, sim_params in zip( # Decide here: zip or product
#             ground_truth_ext_flow_params_dynamic_list,
#             ground_truth_sim_params_dynamic_list
#         ):
#             # Instantiate model with ground truth internal parameters
...
if __name__ == "__main__":
    ...
    # TODO Can be generalised into a function, depending on the parameters one wants to infer
    ### Loop through Beta, A --> Generalised to loop over any parameters (except sim params for now)
    ...
    if True:
        ...
        # Ground truth fixed internal parameters
        ground_truth_fixed_int_params = {} # {'Sp4': 1} # Remark: N, tau_b, tau_s, etc., will be generated with default values.

        ext_param_range = {
            'A': np.pow(10, np.linspace(start=-4, stop=0, num=50)),
        }
        # Ground truth fixed external parameters
        ground_truth_fixed_ext_params = {'w0': 0, 'psi':np.pi/2}             

        # Create and save index mappings
        base_path = Path(__file__).resolve().parent.parent / 'Inference' / 'FromSimulationData' / 'ElasticInference_ShearElasticity'
        base_path.mkdir(parents=True, exist_ok=True)  

        # Combine all parameter mappings into a single dictionary
        all_mappings = {}
        for param_name, param_values in {**int_param_range, **ext_param_range}.items():
            all_mappings[param_name] = {i: float(val) for i, val in enumerate(param_values)}
        ...
        # =======================
        # ===== Nested Loop ===== # TODO Can be put into a function
        # =======================

        # Create all combinations of internal, external, and initial guess parameter indices
        int_param_names = list(int_param_range.keys())
        ext_param_names = list(ext_param_range.keys())

        int_param_indices_iter = product(*[range(len(int_param_range[name])) for name in int_param_names])
        ext_param_indices_iter = product(*[range(len(ext_param_range[name])) for name in ext_param_names])
        
        for int_param_indices in int_param_indices_iter:
            # Build internal parameter dict and folder name parts
            int_param_dict = {}
            ...
            for ext_param_indices in ext_param_indices_iter:
                # Build external parameter dict and folder name parts
                ext_param_dict = {}
                folder_name_parts_ext = []
                
                for param_name, idx in zip(ext_param_names, ext_param_indices):
                    value = ext_param_range[param_name][idx]
                    ext_param_dict[param_name] = value
                    folder_name_parts_ext.append(f'{param_name}_{idx:03d}')
                
                folder_name = '_'.join(folder_name_parts_int + folder_name_parts_ext)
                print(f"{folder_name}: int={int_param_dict}, ext={ext_param_dict}")

                # ============================
                # ======= Ground truth ======= # TODO Can be put into a function (except cumul) 
                # ============================
                
                # Merge fixed and looped internal parameters
                ground_truth_int_params = {
                    **ground_truth_fixed_int_params,
                    **int_param_dict,
                }
                ground_truth_int_params = make_ground_truth_int_params(**ground_truth_int_params)
                ...
                ground_truth_ext_params_list = [make_ground_truth_ext_params(**ground_truth_ext_params)]
                
                # # TODO Modify this to correspond to what I want it to do.
                # # Cumulative version (example: cumulative over first external parameter if it exists)
                # ground_truth_ext_params_list_cumul = []
                # if ext_param_names:
                #     cumul_param_name = ext_param_names[0]  # Cumulate over first external param
                #     cumul_idx = ext_param_indices[0]
                    
                #     for j in range(cumul_idx + 1):
                #         cumul_ext_params = ext_param_dict.copy()
                #         cumul_ext_params[cumul_param_name] = ext_param_range[cumul_param_name][j]
                #         ground_truth_ext_params_list_cumul.append(cumul_ext_params)
                # else:
                #     ground_truth_ext_params_list_cumul = ground_truth_ext_params_list
                ...
                # TODO Modify so that guesses are automatically separated based on inference pass (Elastic vs Viscous)
                ...
                # TODO Modify so that Models are automatically separated based on inference pass (Elastic vs Viscous)
                
                param_keys_to_infer = list(initial_guesses_per_pass[0][0].keys())
                ...
                # TODO Can be put into a function make_optimizer_bounds
                lb = [0 if ('Beta' in param_key or 'tau_b' in param_key or 'tau_s' in param_key) 
                ...
                # TODO Can be generalised to Elastic -> Viscous Inference based on param_keys_to_infer
                ...
                result = pipeline.run(initial_guesses_per_pass, verbose=True)[0]
                # result_cumul = pipeline_cumul.run(initial_guesses_per_pass, verbose=True)[0]
                
                # ============================
                # =========== Save ===========
                # ============================
                
                pair_folder = base_path / folder_name
                pair_folder.mkdir(parents=True, exist_ok=True)
                
                result_file = pair_folder / 'result.pkl'
                result.save(str(result_file.resolve()))
                
                result_cumul_file = pair_folder / 'result_cumul.pkl'
                # result_cumul.save(str(result_cumul_file.resolve()))